...
// points at the kind byte; width is 64 or 32 (float64 / float32 variant).
func (d *Decoder) skipALPRD(width int) error {
	d.i++ // kind
	n64, nr := readUvarint(d.buf[d.i:])
	if nr <= 0 {
	...
	// Bound n by the remaining bytes BEFORE any multiply: 2 bytes/value is the
	// floor for both widths (float32: 16 right bits; float64: >= 48). A larger
	// n could wrap (width-leftBW)*n and turn a huge claim into a tiny skip.
	if n64 > uint64(len(d.buf)-d.i)/2+1 {
...
// --- float32 variant -------------------------------------------------------
//
// Same construction over the 32-bit pattern: left cut in [8,16] bits, right
// 16..24 bits. Model weights and embeddings — the dominant float32 payloads —
// concentrate sign+exponent into a handful of left patterns, which is exactly
...
func alprdPlanFloat32(s []float32) (plan alprdPlan, estBytes int, ok bool) {
	...
	exc := 0
	for _, v := range s {
		left := uint16(math.Float32bits(v) >> shift)
		if !slices.Contains(dict, left) {
			exc++
		}
	}
	...
	rightBody := (int(32-plan.leftBW)*n + 7) / 8
	codeBody := (int(plan.cbits)*n + 7) / 8
	estBytes = 2 + uvarintLen(uint64(n)) + 2 + 2*int(plan.dictN) +
		codeBody + rightBody + uvarintLen(uint64(exc)) + exc*(5+2)
	if estBytes >= 2+uvarintLen(uint64(n))+4*n {
		return plan, 0, false
	}
	return plan, estBytes, true
}

// writePackedALPRDFloat32Slice emits s under the float32 ALP-RD codec.
func (e *Encoder) writePackedALPRDFloat32Slice(s []float32, plan alprdPlan) {
	e.writeHeader()
	n := len(s)
	rightBW := 32 - int(plan.leftBW)
	rightBody := (rightBW*n + 7) / 8
	codeBody := (int(plan.cbits)*n + 7) / 8
	out := slices.Grow(e.buf, 2+10+2+2*int(plan.dictN)+codeBody+rightBody+10+plan.exc*7)
	out = append(out, tagPackALP, qpackKindALPRD32)
	out = appendUvarint(out, uint64(n))
	out = append(out, plan.leftBW, plan.dictN)
	for _, dv := range plan.dict[:plan.dictN] {
		out = append(out, byte(dv), byte(dv>>8))
	...
	shift := uint(rightBW)
	rightMask := uint32(1)<<shift - 1
	dict := plan.dict[:plan.dictN]
	for i, v := range s {
		b := math.Float32bits(v)
		rights[i] = uint64(b & rightMask)
		left := uint16(b >> shift)
		code := 0
		found := false
		for j, dv := range dict {
...
// readPackedALPRDFloat32Slice decodes a float32 ALP-RD body. The tag must
// already be consumed and the next byte must be qpackKindALPRD32.
func (d *Decoder) readPackedALPRDFloat32Slice() ([]float32, error) {
	...
	// Every value costs >= 16 right bits on a valid stream.
	if rem := uint64(len(d.buf) - d.i); n64 > rem/2+1 {
		return nil, ErrShortBuffer
	}
	if n64 > uint64(math.MaxInt) {
		return nil, ErrInvalidLength
	...
	var dict [alprdMaxDict]uint32
	for j := range dictN {
		dict[j] = uint32(d.buf[d.i]) | uint32(d.buf[d.i+1])<<8
		d.i += 2
	}
	...
	for i := range out {
		...
		out[i] = math.Float32frombits(dict[c]<<shift | uint32(rights[i]))
	}
	...
	for range int(excN64) {
		...
		left := uint32(d.buf[d.i]) | uint32(d.buf[d.i+1])<<8
		d.i += 2
		i := int(pos64)
		out[i] = math.Float32frombits(left<<shift | uint32(rights[i]))
	}
	return out, nil
}
