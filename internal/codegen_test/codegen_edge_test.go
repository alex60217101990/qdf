...
// TestCodegen_EmbeddedRoundtrip exercises the three shapes the generator used
// to mis-emit: an anonymous embedded value-struct (flattened into the parent
// wire layout), a map keyed by a defined string type (intern fast path needs a
// conversion), and a pointer to a defined scalar. It round-trips through the
// generated methods and cross-checks interop with the reflect path both ways.
func TestCodegen_EmbeddedRoundtrip(t *testing.T) {
	lbl := Label("ptr-label")
	in := Edge{
		EmbeddedBase: EmbeddedBase{BaseA: 7, BaseB: "embedded"},
		Labels:       map[Label]int{"only": 42}, // single entry → deterministic wire
		Ptr:          &lbl,
		Tail:         99,
	}
	...
	mustUnmarshalQDF(t, b, &out)
	if !equalEdge(in, out) {
		t.Fatalf("generated round-trip mismatch:\n in =%+v\n out=%+v", in, out)
	}

	// Generated -> reflect: the flattened embedded fields must appear at the
	// top level for the reflect decoder, proving wire compatibility.
	var ref Edge
	if err := qdf.Unmarshal(b, &ref); err != nil {
		t.Fatalf("qdf.Unmarshal of generated bytes: %v", err)
	}
	if !equalEdge(in, ref) {
		t.Fatalf("generated->reflect mismatch:\n in =%+v\n ref=%+v", in, ref)
	}

	// Reflect -> generated: the reverse direction.
	rb, err := qdf.Marshal(&in, qdf.OptSpeed)
	if err != nil {
		t.Fatalf("qdf.Marshal(OptSpeed): %v", err)
	}
	var fromRef Edge
	mustUnmarshalQDF(t, rb, &fromRef)
	if !equalEdge(in, fromRef) {
		t.Fatalf("reflect->generated mismatch:\n in =%+v\n out=%+v", in, fromRef)
	}
}

// TestCodegen_EmbeddedWireParity asserts byte-for-byte parity between the
// generated MarshalQDF and qdf.Marshal(OptSpeed) for the embedded-struct
// layout. A non-flattened generator would key the embedded fields under a
// nested "EmbeddedBase" sub-map and diverge here. The map is left nil to keep
// the output order deterministic (map ranging is unordered).
func TestCodegen_EmbeddedWireParity(t *testing.T) {
	lbl := Label("x")
	in := Edge{
		EmbeddedBase: EmbeddedBase{BaseA: 3, BaseB: "b"},
		Labels:       nil,
		Ptr:          &lbl,
		Tail:         5,
	}
	...
	if string(genBytes) != string(reflectBytes) {
		t.Fatalf("generated != reflect for embedded layout:\n gen    =%x\n reflect=%x", genBytes, reflectBytes)
	}
}

func equalEdge(a, b Edge) bool {
	if a.EmbeddedBase != b.EmbeddedBase || a.Tail != b.Tail {
		return false
	}
