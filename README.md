...
dictionary, Gorilla/Chimp128 XOR, FSST, tANS/FSE), so it shrinks *across* records
...
| **Entropy coding (ANS)** — push the encoded stream to its Shannon limit. | Shipped behind `OptRANS` (in `OptCompression`): a static order-0 tANS/FSE entropy pass (table-driven Finite State Entropy; the `OptRANS` name is kept for compatibility) over the whole body, applied only when it shrinks (never larger). Squeezes the residual byte-entropy the structural codecs leave — e.g. −37 % on trace batches — at a fraction of the old rANS coder's CPU (3.2× faster encode, 2–3× faster decode), so it stays in the opt-in compression tier. Legacy rANS blobs still decode — the decoder dispatches on the blob tag byte. |
...
| `0xE7`               | QPack: Gorilla / Chimp128 XOR-coded float slice (kind byte selects) |
...
| `qdf.OptCompression` | `OptBalanced` plus the heavier wire-size codecs: the Gorilla / Chimp128 XOR pair (best-of picker) for smooth float series, ALP for quantized/decimal `[]float64` and `[]float32`, FSST substring-level compression for high-cardinality string columns (URLs, log lines, paths), **hybrid columnar** (transposes the eligible columns of a *mixed* struct — one with a map / slice / nested field — keeping the rest row-major, so log / AD / span records get columnar treatment they otherwise couldn't), and a final order-0 tANS/FSE entropy pass over the whole body (table-driven Finite State Entropy; never larger; the `OptRANS` name is kept for compatibility). Trades encode CPU for smaller wire — pick it for backup / cold storage. |
...
| `OptCompression` | `OptBalanced` + the Gorilla / Chimp128 XOR pair (best-of picker) + ALP decimal floats (`[]float64` / `[]float32`) + FSST for high-cardinality string columns + an order-0 tANS/FSE entropy pass (never larger; the `OptRANS` name is kept for compatibility). Trades CPU for wire size; for backup / cold storage. |
...
| Gorilla / Chimp128 | float slices (explicit opt-in via low-level API) | Best-of pair, encoder keeps the smaller blob. Gorilla: XOR with previous, run-length leading/meaningful-bit window (Facebook VLDB 2015). Chimp128: XOR against the best of the previous 128 values via a low-mantissa hash window, 2-bit control codes (Liakos et al., VLDB 2022). `float32` is Gorilla-only. |
